...
router.get('/pointInfo/:pointID', function (req, res) {
    ...
    DButilsAzure.execQuery(`select c.categoryName , p.* from points as p join category as c on c.categoryID=p.categoryID where p.pointID='` + pointID + `'`)
...
router.get('/popPointInLoad/:certainRate', function (req, res) {
    ...
    let query = `select p.*, c.categoryName from points as p join category as c on c.categoryID=p.categoryID where p.rate >` + num;
    console.log(query);
        ...
        .then((response) => {
            ...
            if (answers[0].ans1.localeCompare(securityAns1) == 0) {
                if (answers[0].ans2.localeCompare(securityAns2) == 0) {
                    ...
                    res.send("you'r second answer is incorrect, try again!");
            }
            else
                res.send("you'r first answer is incorrect, try again!");
        })
        .catch((err) => {
...
router.post('/login', function (req, res) {
    ...
    let token;
    if (!userID || !password)
        res.send({ success: false, message: "bad values" })
    else {
...
router.get('/pointsAccCat/:category', function (req, res) {
    ...
    DButilsAzure.execQuery(`select p.*, c.categoryName from points as p join category as c on c.categoryID=p.categoryID where c.categoryName='` + nameCat + `'`)
        .then((response) => {
            // console.log(response);
            res.send(response);
        })
...
router.get('/points', function (req, res) {
    DButilsAzure.execQuery(`select p.*, c.categoryName from points as p join category as c on c.categoryID=p.categoryID order by p.categoryID asc`)
        .then((response) => {
            console.log(response);
            res.send(response);
        })
...
//localhost:3000/guests/countries
router.get('/countries', function (req, res) {
    var countriesInArray = new Array();
    var xmlConvert = convert.xml2json(xml, { compact: true, spaces: 4 });
    var XMLfile = JSON.parse(xmlConvert);

    for (let i = 0; i < XMLfile.Countries.Country.length; i++) {
        countriesInArray[i] = XMLfile.Countries.Country[i].Name._text;
    }
    res.send(countriesInArray);
