...
router.get('/favPoints', function (req, res) {
    ...
    DButilsAzure.execQuery(`select p.*, f.priority from (select p1.*, c.categoryName from points as p1 join category as c on c.categoryID=p1.categoryID) as p join favorites as f on f.pointID=p.pointID where f.userID='` + userID + `'`)
        .then((response) => {
...
//10
//localhost:3000/auth/users/2favPoints
router.get('/2favPoints', function (req, res) {
    let token= req.decoded;
    //console.log(token);   
    let userID = token.userName;
    let queryCount;
    DButilsAzure.execQuery(`select count(pointID) numOfFav from favorites where userID='` + userID + `'`)
        .then((response) => {
            ...
            if (queryCount > 0) {
                DButilsAzure.execQuery(`select top 2 p.* from favorites as f join (select p1.*, c.categoryName from points as p1 join category as c on c.categoryID=p1.categoryID) as p on p.pointID=f.pointID where userID='` + userID + `' order by f.date desc`)
                    .then((response) => {
                        console.log(response);
                        res.send(response);
                    })
...
router.post('/pointAsFav', function (req, res) {
    ...
    let values = `(CAST((GETDATE()) AS DATE),'` + userID + `','` + pointID + `','` + priority+`')`;
    let queryInsert = `INSERT INTO favorites (date, userID, pointID, priority) VALUES` + values;
...
router.post('/ratePoint', function (req, res) {
    ...
    DButilsAzure.execQuery(query)
        .then((response) => {
            ...
            let avg1=Math.round(avg);
            DButilsAzure.execQuery(`update points set rate=` + (avg1) + ` where pointID='` + pointID + `'`);
...
router.post('/priority', function (req, res) {
    ...
    let token= req.decoded;  
    let userID = token.userName;
    DButilsAzure.execQuery(`update favorites set priority=` + (priority) + ` where pointID='` + pointID + `' and userID='`+userID+`'`) 
    .then((response) => {
        res.send(true);
